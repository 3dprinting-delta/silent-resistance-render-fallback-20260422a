...
const CONTACT_EMAIL = process.env.CONTACT_EMAIL || "375351feng@gmail.com";
...
async function handleApi(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/api/meshy/start") {
    ...
    const paymentConfirmation = String(body.paymentConfirmation || "").trim();
    if (!prompt) return badRequest(res, "A design prompt is required.");
    if (!paymentConfirmation) {
      return badRequest(res, "Payment confirmation is required before starting a Meshy task.");
    }
  ...
  if (req.method === "POST" && url.pathname === "/api/automation/daily-meshy") {
    ...
    const result = await runDailyMeshyDraftJob({
      dataDir: DATA_DIR,
      maxCredits: Number(body.maxCredits || process.env.MESHY_DAILY_CREDIT_BUDGET || 5),
      force: Boolean(body.force),
      dryRun: body.dryRun !== undefined ? Boolean(body.dryRun) : process.env.MESHY_DAILY_AUTOMATION_ENABLED !== "true",
